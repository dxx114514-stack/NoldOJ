...
- **冷却机制**：每用户每题 60 秒，防止滥用
- **智能提示词**：AI 扮演"温柔算法教练"，根据用户提交历史给出针对性方向建议

### AI 测试数据生成
- **一键生成**：教师可在管理面板为指定题目自动生成测试用例（正常用例 + 边界用例）
- **参数可控**：可指定正常用例数（1-10）和边界用例数（0-10）
- **磁盘写入**：生成的测试用例自动写入 testdata 目录并入库
...
### v2.1.0
- **新增试卷系统**：支持选择题、判断题、填空题（客观/主观）、大题四种题型
- 试卷 CRUD：教师可创建/编辑/删除试卷，配置时间限制、及格分、最大尝试次数等
